...
from src.algorithms.LogisticRegressionAlgorithm import LogisticRegressionAlgorithm, default_logistic_regression
from src.algorithms.HistGradientBoostingAlgorithm import HistGradientBoostingAlgorithm, \
    default_hist_gradient_boosting_classifier
from src.algorithms.NuSVCAlgorithm import NuSVCAlgorithm, default_NuSVC, default_NuSVC_params
...
from src.algorithms.RandomForestClassifier import default_randomforest_df
...
class Algorithm:
    ...
    def ml_algo_RandomForest(self, X, y, nom):
        print(nom)
        params_csv = default_randomforest_df()
    ...
    def ml_algo_LogisticRegression(self, X, y, nom):
        params_csv = default_logistic_regression()
    ...
    def ml_algo_HistGradientBoosting(self, X, y, nom):
        params_csv = default_hist_gradient_boosting_classifier()
    ...
    def ml_algo_NuSVC(self, X, y, nom):
        print('ml_algo')
        params_csv = default_NuSVC_params()

        os.chdir('..')
        nom_fichier = 'NuSVC_params.csv'
        params_csv = params_csv.append(pd.read_csv(nom_fichier, index_col='index'))
        fichier_sel_k_best = 'Select_KBest_params.csv'
        select_k_best_csv = get_params_from_file(fichier_sel_k_best)
        os.chdir('AutoML')
        ...
        for it_kbest, row_kbest in select_k_best_csv.iterrows():
            ...
            for it_i, row in params_csv.iterrows():
                ...
                dossier = nom + '_' + num + '_' + 'KBest_' + str(row_kbest['k'])
                print('\n***************\n' + dossier)
                c: NuSVCAlgorithm = NuSVCAlgorithm(b.X_train.to_numpy(),
                                                   b.y_train.iloc[:, 0].ravel(), row)
                print('C')
                c.fit_clf(b.X_train, b.y_train)
                if dossier not in os.listdir():
